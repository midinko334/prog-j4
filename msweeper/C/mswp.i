...
int board_sel(int **cell,int size,int xy){
  ...
  while(1){
    ...
    if(input=='s'&&xy+size<((size)*(size))) xy+=size;
...
int opencell_near(int **cell,int x,int y,int size){
  ...
  if(count==cell[y][x]) for(int j=-1;j<2;j++) for(int k=-1;k<2;k++)
   if((x+k>=0&&x+k<size)&&(y+j>=0&&y+j<size)){
    ...
    else if(cell[y+j][x+k]==-1) return 0;
  }
  return 2;
}
...
void gameloop(int **cell,int size,int xy){
  unsigned int sttime=(unsigned)time(
# 141 "mswp.c" 3 4
                                    ((void *)0)
# 141 "mswp.c"
                                        );
  ...
  while(1){
    ...
    if(cell[y][x]==9||cell[y][x]==11) opencell(cell,x,y,size);
    if(cell[y][x]>=1&&cell[y][x]<=8){
      finflag=opencell_near(cell,x,y,size);
      if(finflag==0) break;
    }
    else if(cell[y][x]==-1||cell[y][x]==-3){
      finflag=0;
      break;
    }
  }
...
# 174 "mswp.c"
  ...
  (*cell)=(int**)malloc(sizeof(int*)*(size));
  for(int i=0;i<size;i++) (*cell)[i]=(int*)malloc(sizeof(int)*(size));
  senkei=(int*)malloc(sizeof(int)*((size)*(size)));
  for(int i=0;i<size;i++) for(int j=0;j<size;j++) (*cell)[i][j]=9;
  for(int i=0;i<((size)*(size));i++) senkei[i]=i;
  for(int i=((size)*(size))-1;i>0;i--) swap(senkei+( rand()%(i+1) ),senkei+i);

  int input,cur=0;
  input=board_sel((*cell),size,0);
  for(int i=0;i<((size)*(size));i++) if(senkei[i]==input) swap(&senkei[i],&senkei[((size)*(size))-1]);
  for(int i=0;i<((size)*(size))-1-cur;i++){
    for(int j=-1;j<2;j++) for(int k=-1;k<2;k++)
     ...
     if(senkei[i]%size==input%size+k && senkei[i]/size==input/size+j){
      swap(&senkei[i],&senkei[((size)*(size))-2-cur]);
      cur++;
      i--;
    }
  }
...
int main(){
  ...
  while(size<2||size>50){
    printf("Board size (Max %d Min 2): ",50);
    scanf("%d",&size);
    while((c = getchar()) != '\n' && c != 
# 217 "mswp.c" 3 4
                                         (-1)
# 217 "mswp.c"
    ...
    if(size<2||size>50) printf("Invalid Input\n");
  }
  while(mine>=((size)*(size))||mine<=0){
    printf("Mine count : ");
    scanf("%d",&mine);
    while((c = getchar()) != '\n' && c != 
# 223 "mswp.c" 3 4
...
# 223 "mswp.c"
    ...
    if(mine>=((size)*(size))||mine<=0) printf("Invalid Input\n");
  }
