...
.done:
    movzx ebp, al
    movzx edi, ah
    lea eax, [rbp + rdi*8]         ; index = y*8 + x, computed directly into eax

    cmp byte [r15 + 98 + rax], 1  ;(cmp byte [opened + rax], 1)
    je main_loop
    cmp byte [r15 + rax], 1       ;(cmp byte [mine + rax], 1)
    je .gameover
    mov byte [r15 + 98 + rax], 1  ;(mov byte [opened + rax], 1)
    inc r13d
